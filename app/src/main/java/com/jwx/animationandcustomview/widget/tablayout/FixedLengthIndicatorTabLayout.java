...
package com.jwx.animationandcustomview.widget.tablayout;
...
import com.jwx.animationandcustomview.widget.tablayout.PagerAdapter;
...
public class FixedLengthIndicatorTabLayout extends HorizontalScrollView {
    ...
    private static final int INDICATOR_WIDTH = 40; //新添代码，指针宽度
    private static final int ANIMATIOR_TIME = 100; //指针动画执行时间
    ...
    /**
     * Gravity used to fill the {@link FixedLengthIndicatorTabLayout} as much as possible. This option only takes effect
    ...
    /**
     * Gravity used to lay out the tabs in the center of the {@link FixedLengthIndicatorTabLayout}.
     *
     * @see #setTabGravity(int)
     * @see #getTabGravity()
     */
    ...
    private int lastPosition; //记录上一个页面角标
    private static int positionToLeft;//向左滑动时，页面角标规律特殊，需额外标记
    private static boolean isLeftScroll = false; //正在 左滑
    private static boolean isRightScroll = false;//正在右滑
    ...
    public FixedLengthIndicatorTabLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public FixedLengthIndicatorTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    /**
     * Returns the current mode used by this {@link FixedLengthIndicatorTabLayout}.
     *
     * @see #setTabMode(int)
     */
    @Mode
    ...
    /**
     * The one-stop shop for setting up this {@link FixedLengthIndicatorTabLayout} with a {@link ViewPager}.
     * <p>
    ...
    private void setupWithViewPager(@Nullable final ViewPager viewPager, boolean autoRefresh,
        ...
        if (viewPager != null) {
            ...
            lastPosition = viewPager.getCurrentItem(); //新添代码，设置lastPosition初始值，确保滚动动画初始值正确
        } else {
    ...
    private void animateToTab(final int newPosition) { //tab动画
        ...
        // Now animate the indicator  TODO 新添代码，指针动画 ,此处设置Indicator滚动动画(以下)
        View lastTab = mTabStrip.getChildAt(lastPosition);
        View nowTab = mTabStrip.getChildAt(newPosition);
        int tempLenght = (lastTab.getWidth()
                - DisplayUtil.dip2px(getContext(),INDICATOR_WIDTH)) / 2;//新添代码，左右差值
        final int lastLeft = lastTab.getLeft() + tempLenght;
        final int lastRight = lastTab.getRight() - tempLenght;
        int nowLeft = nowTab.getLeft() + tempLenght;
        int nowRight = nowTab.getRight() - tempLenght;
        final ValueAnimator animator;
        if (!isDragging) {
            if (lastPosition < newPosition) {
                //向左滚，左边不动值不变
//                Log.e("jwx", "向左滚:   lastPosition=" + lastPosition + ";newPosition=" + newPosition);
                ...
                animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                    ...
                    public void onAnimationUpdate(ValueAnimator animation) {
                        ...
                        mTabStrip.setIndicatorPosition(lastLeft, value);
                    }
                });
                animator.setDuration(ANIMATIOR_TIME).start();
            } else {
                //向右滚，右边不动值不变
//                Log.e("jwx", "向右滚:   lastPosition=" + lastPosition + ";newPosition=" + newPosition);
                animator = ValueAnimator.ofInt(
                        lastLeft, nowLeft);
                animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                    ...
                    public void onAnimationUpdate(ValueAnimator animation) {
                        // 获取动画当前的值
                        int value = (Integer) animator.getAnimatedValue();
                        mTabStrip.setIndicatorPosition(value, lastRight);
                    }
                });
                animator.setDuration(ANIMATIOR_TIME).start();
            }
        ...
        // Now animate the indicator  指针动画 ,此处设置Indicator滚动动画(以上)
//        mTabStrip.animateIndicatorToPosition(newPosition, ANIMATION_DURATION); //源码，控制指针滚动动画
    ...
    public static final class Tab {
        ...
        FixedLengthIndicatorTabLayout mParent;
        TabView mView;
    ...
    private class SlidingTabStrip extends LinearLayout {
        ...
        float lastSelectionOffset = 1; //新添代码，记录上一次位移值
        ...
        private void updateIndicatorPosition() {
            ...
            if (selectedTitle != null && selectedTitle.getWidth() > 0) {
                //TODO 修改left right值来改变indicator长度
                int tempLenght = (selectedTitle.getWidth()
                        - DisplayUtil.dip2px(getContext(),INDICATOR_WIDTH)) / 2;//新添代码，左右差值
                left = selectedTitle.getLeft() + tempLenght;
                right = selectedTitle.getRight() - tempLenght;
                if (mSelectionOffset > 0f && mSelectedPosition < getChildCount() - 1) {
                    // Draw the selection partway between the tabs TODO 此处修改滑动过程中指针长度
                    View nextTitle = getChildAt(mSelectedPosition + 1);
                    //源码，控制指针跟随滑动
//                    left = (int) (mSelectionOffset * (nextTitle.getLeft()+tempLenght) +
//                            (1.0f - mSelectionOffset) * left);
//                    right = (int) (mSelectionOffset * (nextTitle.getRight()-tempLenght) +
//                            (1.0f - mSelectionOffset) * right);
                    //新添代码，控制指针展示粘性滑动效果(以下)
                    if (lastSelectionOffset >= mSelectionOffset ||
                            (lastSelectionOffset == 0 && mSelectionOffset > 0.5)) {
                        //向右滑动
                        if (!isLeftScroll) {
                            isRightScroll = true;
                            left = (int) (mSelectionOffset * (2 * mSelectionOffset - 1) *
                                    (nextTitle.getLeft() + tempLenght) + (1.0f - mSelectionOffset *
                                    (2 * mSelectionOffset - 1)) * left);
                            if (left <= selectedTitle.getLeft() + tempLenght) {
                                left = selectedTitle.getLeft() + tempLenght;
                                right = (int) (mSelectionOffset * 2 * (nextTitle.getRight() - tempLenght) +
                                        (1.0f - mSelectionOffset * 2) * right);
                            } else {
                                right = nextTitle.getRight() - tempLenght;
                            }
//                            Log.e("jwx", "向右滑动   left=" + left + ";right=" + right +
//                                    ";mSelectionOffset=" + mSelectionOffset + ";lastSelectionOffset="
//                                    + lastSelectionOffset + ";mSelectedPosition = " + mSelectedPosition);
                        } else {
                            left = (int) (mSelectionOffset * (2 * mSelectionOffset - 1) *
                                    (nextTitle.getLeft() + tempLenght) + (1.0f - mSelectionOffset *
                                    (2 * mSelectionOffset - 1)) * left);
                            right = (int) (mSelectionOffset * 2 * (nextTitle.getRight() - tempLenght) +
                                    (1.0f - mSelectionOffset) * right);
                            if (right >= nextTitle.getRight() - tempLenght) {
                                right = nextTitle.getRight() - tempLenght;
                            } else {
                                left = selectedTitle.getLeft() + tempLenght;
                            }
//                            Log.e("jwx", "向左滑动时发生了右滑   left=" + left + ";right=" + right +
//                                    ";mSelectionOffset=" + mSelectionOffset + ";lastSelectionOffset="
//                                    + lastSelectionOffset + ";mSelectedPosition = " + mSelectedPosition);
                        }
                    } else {
                        ...
                        if (!isRightScroll) {
                            ...
                            if (right > nextTitle.getRight() - tempLenght) {
                                ...
                                left = (int) (mSelectionOffset * (2 * mSelectionOffset - 1) *
                                        (nextTitle.getLeft() + tempLenght) + (1.0f - mSelectionOffset *
                                        (2 * mSelectionOffset - 1)) * left);
                                if (left >= nextTitle.getLeft() + tempLenght) {
                                    left = nextTitle.getLeft() + tempLenght;
                                }
                                if(left <= selectedTitle.getLeft() + tempLenght){
                                    left = selectedTitle.getLeft() + tempLenght;
                                }
                            }
//                            Log.e("jwx", "向左滑动   left=" + left + ";right=" + right +
//                                    ";mSelectionOffset=" + mSelectionOffset + ";lastSelectionOffset="
//                                    + lastSelectionOffset + ";mSelectedPosition = " + mSelectedPosition);
                        } else {
                            left = (int) (mSelectionOffset * (2 * mSelectionOffset - 1) *
...
//                            Log.e("jwx", "向右滑动时发生了左滑   left=" + left + ";right=" + right +
//                                    ";mSelectionOffset=" + mSelectionOffset + ";lastSelectionOffset="
//                                    + lastSelectionOffset + ";mSelectedPosition = " + mSelectedPosition);
                        }
                    }
                    //新添代码，控制指针展示粘性滑动效果(以上)
                }
            } else {
                left = right = -1;
            }
            ...
            //TODO 此处添加滑动页面结束后的动画
//            if (isDragging && mSelectionOffset == 0f && selectedTitle != null) {
//                final ValueAnimator animator;
//                if (mSelectedPosition < getChildCount() - 1) {
//                    if (lastSelectionOffset < 0.5) {
//                        //向右滑动
//                        if(isRightScroll){
//                            View lastTab = getChildAt(mSelectedPosition);
//                            View nextTitle = getChildAt(mSelectedPosition + 1);
//                            int tempLenght = (lastTab.getWidth()
//                                    - DisplayUtil.dip2px(INDICATOR_WIDTH)) / 2;//新添代码，左右差值
//                            final int lastLeft = lastTab.getLeft() + tempLenght;
//                            int lastRight = lastTab.getRight() - tempLenght;
//                            int nowRight = nextTitle.getRight() - tempLenght;
//                            Log.e("jwx", "向右滑动结束：   " + "lastRight=" + lastRight +
//                                    ";nowRight=" + nowRight + ";mSelectedPosition=" + mSelectedPosition);
//                            animator = ValueAnimator.ofInt(
//                                    nowRight, lastRight);
//                            animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
//
...
//                        //向左滑动
//                        if(isLeftScroll){
//                            Log.e("jwx", "执行了向左滑动滑动结束时的动画******************************");
//                            View lastTab = getChildAt(positionToLeft - 1);
//                            View nextTitle = getChildAt(positionToLeft);
//                            int tempLenght = (lastTab.getWidth()
//                                    - DisplayUtil.dip2px(INDICATOR_WIDTH)) / 2;//新添代码，左右差值
//                            final int lastLeft = lastTab.getLeft() + tempLenght;
//                            final int nowRight = nextTitle.getRight() - tempLenght;
//                            Log.e("jwx", "向左滑动结束：   " + "lastLeft=" + lastLeft +
//                                    ";nowRight=" + nowRight + ";mSelectedPosition=" + mSelectedPosition + ";positionToLeft=" + positionToLeft);
//                            int nowLeft = nextTitle.getLeft() + tempLenght;
//                            animator = ValueAnimator.ofInt(
//                                    lastLeft, nowLeft);
//                            animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
//
...
//                                    // 获取动画当前的值
//                                    int value = (Integer) animator.getAnimatedValue();
//                                    mTabStrip.setIndicatorPosition(value, nowRight);
//                                }
//                            });
//                            animator.setDuration(ANIMATIOR_TIME).start();
//                        } else {
...
//                        @Override
//                        public void onAnimationEnd(Animator animation) {
//                            mTabStrip.animateIndicatorToPosition(positionToLeft, ANIMATION_DURATION);
//                            lastPosition = positionToLeft;
//                            super.onAnimationEnd(animation);
//                        }
//                    });
//                } else if (mSelectedPosition == getChildCount() - 1) {
//                    //向左滑动
//                    if (isLeftScroll){
//                        Log.e("jwx", "执行了向左滑动滑动结束时的动画******************************");
//                        View lastTab = getChildAt(positionToLeft - 1);
...
//            }
            ...
            lastSelectionOffset = mSelectionOffset; //新添代码，更新上次位移值
            lastPosition = mSelectedPosition;//新添代码，更新页面角标
        }
        ...
        void animateIndicatorToPosition(final int position, int duration) {
            ...
            final boolean isRtl = ViewCompat.getLayoutDirection(this)
                    == ViewCompat.LAYOUT_DIRECTION_RTL; //from Right to Left
            ...
            int tempLenght = (targetView.getWidth()
                    - DisplayUtil.dip2px(getContext(),INDICATOR_WIDTH)) / 2;//新添代码，左右差值
            //final int targetLeft = targetView.getLeft(); 源码
            final int targetLeft = targetView.getLeft() + tempLenght; //新增代码，左右差值
            //final int targetRight = targetView.getRight();源码
            final int targetRight = targetView.getRight() - tempLenght;//新增代码，左右差值
            final int startLeft;
            final int startRight;
            //TODO 此处修改tabIndicator滚动动画
            if (Math.abs(position - mSelectedPosition) <= 1) {
                // If the views are adjacent, we'll animate from edge-to-edge 如果是相邻页面，就执行页间动画
                startLeft = mIndicatorLeft;
                startRight = mIndicatorRight;
//                Log.e("jwx", "the views are adjacent, we'll animate from edge-to-edge;" +
//                        "startLeft=" + startLeft + ";startRight=" + startRight + ";lastPosition=" +
//                        lastPosition + ";position=" + position + ";lastSelectionOffset=" +
//                        lastSelectionOffset + ";mSelectionOffset=" + mSelectionOffset);
            } else {
                // Else, we'll just grow from the nearest edge  否则就从最近的页开始
                final int offset = dpToPx(MOTION_NON_ADJACENT_OFFSET);
                if (position < mSelectedPosition) { //新页码小于旧页码
                    // We're going end-to-start  总方向为向左滚动，页尾到页头位置
                    if (isRtl) {
                        startLeft = startRight = targetLeft - offset; //过程中向左滚动
                    } else {
                        //startLeft = startRight = targetRight + offset; //源码，过程中向右滚动
                        //startLeft = targetRight + offset;//新添代码，设置左边边滚动动画
                        startLeft = mIndicatorLeft;//新添代码，设置左边边滚动动画
                        startRight = mIndicatorRight;//新添代码，设置右边不动
                    }
//                    Log.e("jwx", "We're going end-to-start;" +
//                            "startLeft=" + startLeft + ";startRight=" + startRight + ";isRtl=" + isRtl);
                } else {
                    // We're going start-to-end  总方向为向右滚动，页头到页尾位置
                    if (isRtl) {
                        startLeft = startRight = targetRight + offset;
                    } else {
                        //startLeft = startRight = targetLeft - offset; 源码
                        startLeft = mIndicatorLeft; //新添代码，设置左边不动
                        //startRight = targetLeft - offset;//新添代码，设置右边滚动动画
                        startRight = mIndicatorRight;//新添代码，设置右边滚动动画
                    }
//                    Log.e("jwx", "We're going start-to-end;" +
//                            "startLeft=" + startLeft + ";startRight=" + startRight + ";isRtl=" + isRtl);
                }
            }
        ...
        public void draw(Canvas canvas) {
            ...
            if (mIndicatorLeft >= 0 && mIndicatorRight > mIndicatorLeft) {
                RectF rect = new RectF(mIndicatorLeft, getHeight() - mSelectedIndicatorHeight,
                        mIndicatorRight, getHeight()); //新添代码，创建矩形
//                canvas.drawRect(mIndicatorLeft, getHeight() - mSelectedIndicatorHeight,
//                        mIndicatorRight, getHeight(), mSelectedIndicatorPaint); 源码，绘制指针矩形
                int angleNum = DisplayUtil.dip2px(getContext(),3);
                canvas.drawRoundRect(rect, angleNum, angleNum, mSelectedIndicatorPaint); //新添代码，绘制圆角矩形
            }
        }
    }
     ...
     * necessary calls back to the provided {@link FixedLengthIndicatorTabLayout} so that the tab position is
     * kept in sync.
     * <p>
    ...
    public static class TabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
        private final WeakReference<FixedLengthIndicatorTabLayout> mTabLayoutRef;
        private int mPreviousScrollState;
        private int mScrollState;

        public TabLayoutOnPageChangeListener(FixedLengthIndicatorTabLayout tabLayout) {
...
//            Log.e("jwx", "onPageScrolled  position=" + position + ";positionOffset=" +
//                    positionOffset + ";positionOffsetPixels=" + positionOffsetPixels);
            final FixedLengthIndicatorTabLayout tabLayout = mTabLayoutRef.get();
            if (tabLayout != null) {
